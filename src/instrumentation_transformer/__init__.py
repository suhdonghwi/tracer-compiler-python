...
class InstrumentationTransformer(ast.NodeTransformer):
    ...
    def visit_expr(self, node: ast.expr) -> ast.expr:
        return typing.cast(ast.expr, self.visit(node))

    def visit_stmt(self, node: ast.stmt) -> ast.stmt:
        return typing.cast(ast.stmt, self.visit(node))
    ...
    def visit(self, node: ast.AST) -> ast.AST:
        ...
        if isinstance(node, ast.FunctionDef):
            node.body = list(map(self.visit_stmt, node.body))
            ...
            node.value = self.visit_expr(node.value)
